...
// EchoerPayload is the payload type of the tmess service echoer method.
type EchoerPayload struct {
	// JWT used for authentication
	Token string
}

// ListenerPayload is the payload type of the tmess service listener method.
type ListenerPayload struct {
	// JWT used for authentication
	Token string
}

// SummaryPayload is the payload type of the tmess service summary method.
type SummaryPayload struct {
	// JWT used for authentication
	Token string
}

// ChatSummaryCollection is the result type of the tmess service summary method.
type ChatSummaryCollection []*ChatSummary

// SubscribePayload is the payload type of the tmess service subscribe method.
type SubscribePayload struct {
	// JWT used for authentication
	Token string
}

// Event is the result type of the tmess service subscribe method.
type Event struct {
...
// HistoryPayload is the payload type of the tmess service history method.
...
// ChatSummary is the result type of the tmess service history method.
...
func NewChatSummaryCollection(vres tmessviews.ChatSummaryCollection) ChatSummaryCollection {
...
// view.
func NewViewedChatSummaryCollection(res ChatSummaryCollection, view string) tmessviews.ChatSummaryCollection {
	var vres tmessviews.ChatSummaryCollection
	switch view {
	case "tiny":
		p := newChatSummaryCollectionViewTiny(res)
		vres = tmessviews.ChatSummaryCollection{Projected: p, View: "tiny"}
	case "default", "":
		p := newChatSummaryCollectionView(res)
		vres = tmessviews.ChatSummaryCollection{Projected: p, View: "default"}
	}
	return vres
}
...
// ChatSummary.
func NewChatSummary(vres *tmessviews.ChatSummary) *ChatSummary {
	var res *ChatSummary
	switch vres.View {
	case "tiny":
...
func NewViewedChatSummary(res *ChatSummary, view string) *tmessviews.ChatSummary {
	var vres *tmessviews.ChatSummary
	switch view {
	case "tiny":
		p := newChatSummaryViewTiny(res)
		vres = &tmessviews.ChatSummary{Projected: p, View: "tiny"}
	case "default", "":
		p := newChatSummaryView(res)
		vres = &tmessviews.ChatSummary{Projected: p, View: "default"}
	}
	return vres
}
...
func newChatSummaryCollectionTiny(vres tmessviews.ChatSummaryCollectionView) ChatSummaryCollection {
...
func newChatSummaryCollection(vres tmessviews.ChatSummaryCollectionView) ChatSummaryCollection {
	res := make(ChatSummaryCollection, len(vres))
	for i, n := range vres {
...
func newChatSummaryCollectionViewTiny(res ChatSummaryCollection) tmessviews.ChatSummaryCollectionView {
	vres := make(tmessviews.ChatSummaryCollectionView, len(res))
	for i, n := range res {
...
func newChatSummaryCollectionView(res ChatSummaryCollection) tmessviews.ChatSummaryCollectionView {
	vres := make(tmessviews.ChatSummaryCollectionView, len(res))
	for i, n := range res {
...
// ChatSummary.
func newChatSummaryTiny(vres *tmessviews.ChatSummaryView) *ChatSummary {
	res := &ChatSummary{}
	if vres.Message != nil {
...
// ChatSummary.
func newChatSummary(vres *tmessviews.ChatSummaryView) *ChatSummary {
	res := &ChatSummary{
		Length: vres.Length,
	}
	if vres.Message != nil {
...
func newChatSummaryViewTiny(res *ChatSummary) *tmessviews.ChatSummaryView {
	vres := &tmessviews.ChatSummaryView{
		Message: &res.Message,
	}
	return vres
}
...
func newChatSummaryView(res *ChatSummary) *tmessviews.ChatSummaryView {
	vres := &tmessviews.ChatSummaryView{
		Message: &res.Message,
		Length:  res.Length,
		SentAt:  &res.SentAt,
